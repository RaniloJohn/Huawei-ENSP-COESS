# Clean Enterprise Huawei 32-Lab Suite Generator
...
print(f"Generating {len(labs_data)} clean enterprise UNL labs...")
...
for lab in labs_data:
    ...
    # Build Preloaded Configs XML for EVE-NG
    configs_xml = []
    nodes_xml = []
    for idx, n in enumerate(nodes):
        ...
        if nname in cli_sol:
            cfg_text = "\n".join(cli_sol[nname])
            configs_xml.append(f'''    <config id="{nid}" name="{nname}">{xml_escape(cfg_text)}</config>''')
...
print("All 32 clean enterprise Huawei UNL labs successfully updated!")
