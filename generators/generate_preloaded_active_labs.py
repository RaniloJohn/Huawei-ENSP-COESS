# Clean Enterprise Huawei 32-Lab Suite Generator with True EVE-NG Preloaded Startup-Configs
# EVE-NG Web: admin / eve
# Huawei AR1000v / Devices: admin / admin & super / super
...
# Standard Huawei AAA & Super credential block supporting both admin/admin and super/super
HUAWEI_BASE_AAA = [
    "aaa",
    " local-user admin password cipher admin",
    " local-user admin service-type telnet ssh terminal",
    " local-user admin privilege level 15",
    " local-user super password cipher super",
    " local-user super service-type telnet ssh terminal",
    " local-user super privilege level 15",
    " quit",
    "super password cipher super",
...
def build_clean_enterprise_card(title, desc, table, tasks, cli_solution, verif_steps, card_id="1", left=20, top=20, width=540):
    # Flexbox Device Matrix Rows
    grid_rows = ""
    for r in table:
        dev_name, port, ip_mode, connected_to = r
        grid_rows += f"""
        <div style="display:flex;border-bottom:1px solid #1e293b;padding:4px 0;font-size:11.5px;background:#050811;">
          <div style="width:24%;color:#38bdf8;font-weight:bold;padding:0 4px;">{dev_name}</div>
          <div style="width:20%;color:#94a3b8;padding:0 4px;">{port}</div>
          <div style="width:28%;color:#4ade80;font-weight:bold;padding:0 4px;">{ip_mode}</div>
          <div style="width:28%;color:#f1f5f9;padding:0 4px;">{connected_to}</div>
        </div>
        """

    tasks_html = "".join([f"<li style='margin-bottom:6px;color:#f8fafc;line-height:1.45;'>{t}</li>" for t in tasks])

    # CLI solution blocks without emojis
    cli_html = ""
    for dev, cmds in cli_solution.items():
        cmd_lines = "<br/>".join([c.replace(" ", "&nbsp;") for c in cmds])
        cli_html += f"""
        <div style="margin-top:8px;background:#000000;border:1px solid #1e293b;border-radius:4px;padding:8px 10px;">
          <div style="color:#38bdf8;font-weight:bold;font-size:12px;border-bottom:1px solid #1e293b;padding-bottom:3px;margin-bottom:5px;">CLI - {dev}:</div>
          <div style="font-family:Consolas,Courier New,monospace;color:#4ade80;font-size:11.5px;line-height:1.45;">{cmd_lines}</div>
        </div>
        """
  ...
  <div style="font-family:Segoe UI,Helvetica,Arial,sans-serif;font-size:12px;line-height:1.45;">
    
    <!-- Title Header -->
    <div style="background:#0284c7;color:#ffffff;font-size:13px;font-weight:bold;padding:7px 12px;border-radius:6px 6px 0 0;letter-spacing:0.5px;display:flex;align-items:center;justify-content:space-between;box-shadow:0 4px 6px rgba(0,0,0,0.4);">
      <span>[ {title.replace('-', ' ').upper()} ]</span>
      <span style="font-size:10.5px;background:#0369a1;padding:2px 8px;border-radius:4px;border:1px solid #38bdf8;">Huawei VRP</span>
    </div>

    <!-- Main Card Body -->
    <div style="background:#090d16;border:2px solid #0284c7;border-top:none;border-radius:0 0 8px 8px;padding:10px;box-shadow:0 12px 28px rgba(0,0,0,0.8);">
      
      <!-- Credentials Banner -->
      <div style="background:#1e1b4b;border:1px solid #4338ca;color:#c7d2fe;font-size:11px;padding:5px 8px;border-radius:4px;margin-bottom:8px;display:flex;justify-content:space-between;">
        <span><b>AR1000v Login:</b> admin / admin (or super / super)</span>
        <span><b>EVE-NG:</b> admin / eve</span>
      </div>
      ...
      <details open style="margin-bottom:6px;background:#0f172a;border:1px solid #1e293b;border-radius:5px;">
        <summary style="cursor:pointer;background:#1e293b;color:#38bdf8;font-weight:bold;font-size:12px;padding:6px 10px;border-radius:4px;user-select:none;border:1px solid #38bdf8;">
          [ Lab Scenario &amp; Device Addressing Table ]
        </summary>
        <div style="padding:10px;background:#090d16;">
          <div style="color:#e2e8f0;font-size:12px;margin-bottom:8px;line-height:1.4;">
            <b style="color:#38bdf8;">Scenario:</b> {desc}
          </div>
          
          <div style="display:flex;background:#1e293b;color:#ffffff;font-weight:bold;padding:5px 0;font-size:11.5px;border-radius:4px 4px 0 0;border:1px solid #334155;border-bottom:none;">
            <div style="width:24%;padding:0 4px;color:#38bdf8;">DEVICE</div>
            <div style="width:20%;padding:0 4px;">PORT</div>
            <div style="width:28%;padding:0 4px;color:#facc15;">IP / MODE</div>
            <div style="width:28%;padding:0 4px;">CONNECTED TO</div>
          </div>
          <div style="border:1px solid #334155;border-top:none;border-radius:0 0 4px 4px;background:#050811;">
            {grid_rows}
          </div>
        </div>
      </details>

      <!-- BUTTON 2: TASKS & REQUIREMENTS -->
      <details open style="margin-bottom:6px;background:#0f172a;border:1px solid #1e293b;border-radius:5px;">
...
labs_data.append({
    ...
    "nodes": [
        {"name": "R1-GW", "type": "huaweiar1k", "x": 620, "y": 140, "role": "router"},
        {"name": "ISP-R2", "type": "huaweiar1k", "x": 920, "y": 140, "role": "router"},
        {"name": "PC1", "type": "vpcs", "x": 540, "y": 340, "role": "pc"},
        {"name": "PC2", "type": "vpcs", "x": 700, "y": 340, "role": "pc"}
    ],
    "links": [
        ("R1-GW", 0, "ISP-R2", 0),
        ("R1-GW", 1, "PC1", 0),
    ...
    "tasks": [
        "Configure credentials (admin / admin & super / super) and enable DHCP on R1-GW.",
...
labs_data.append({
    ...
    "nodes": [
        {"name": "Core-R3", "type": "huaweiar1k", "x": 750, "y": 60, "role": "router"},
        {"name": "Master-GW1", "type": "huaweiar1k", "x": 600, "y": 180, "role": "router"},
        {"name": "Backup-GW2", "type": "huaweiar1k", "x": 900, "y": 180, "role": "router"},
        {"name": "SW-Access", "type": "huaweiar1k", "x": 750, "y": 300, "role": "switch"},
        {"name": "PC1", "type": "vpcs", "x": 750, "y": 420, "role": "pc"}
    ],
    "links": [
...
labs_data.append({
    ...
    "nodes": [
        {"name": "Branch-R1", "type": "huaweiar1k", "x": 600, "y": 140, "role": "router"},
        {"name": "HQ-R2", "type": "huaweiar1k", "x": 780, "y": 140, "role": "router"},
        {"name": "Cloud-R3", "type": "huaweiar1k", "x": 960, "y": 140, "role": "router"},
        {"name": "Branch-PC", "type": "vpcs", "x": 600, "y": 320, "role": "pc"},
        {"name": "Server-PC", "type": "vpcs", "x": 960, "y": 320, "role": "pc"}
    ],
    "links": [
        ("Branch-R1", 0, "HQ-R2", 0),
        ("HQ-R2", 1, "Cloud-R3", 0),
...
labs_data.append({
    ...
    "nodes": [
        {"name": "Area1-R1", "type": "huaweiar1k", "x": 600, "y": 140, "role": "router"},
        {"name": "ABR-R2", "type": "huaweiar1k", "x": 780, "y": 140, "role": "router"},
        {"name": "Backbone-R3", "type": "huaweiar1k", "x": 960, "y": 140, "role": "router"},
        {"name": "Area1-PC", "type": "vpcs", "x": 600, "y": 320, "role": "pc"},
        {"name": "Area0-PC", "type": "vpcs", "x": 960, "y": 320, "role": "pc"}
    ],
    "links": [
        ("Area1-R1", 0, "ABR-R2", 0),
...
labs_data.append({
    ...
    "nodes": [
        {"name": "Core-SW1", "type": "huaweiar1k", "x": 620, "y": 140, "role": "switch"},
        {"name": "Dist-SW2", "type": "huaweiar1k", "x": 920, "y": 140, "role": "switch"},
        {"name": "PC1", "type": "vpcs", "x": 620, "y": 320, "role": "pc"},
        {"name": "PC2", "type": "vpcs", "x": 920, "y": 320, "role": "pc"}
    ],
    "links": [
...
# Generate remaining labs 6 to 32
additional_titles = [
...
for lab_item in additional_titles:
    ...
    labs_data.append({
        ...
        "desc": ldesc,
        "nodes": [
            {"name": dev1_name, "type": dev1_type, "x": 620, "y": 140, "role": lrole},
            {"name": dev2_name, "type": dev2_type, "x": 920, "y": 140, "role": lrole},
            {"name": "PC1", "type": "vpcs", "x": 620, "y": 320, "role": "pc"},
            {"name": "PC2", "type": "vpcs", "x": 920, "y": 320, "role": "pc"}
        ],
        "links": [
        ...
        "tasks": [
            f"Configure hostnames and credentials (admin / admin & super / super).",
...
print(f"Generating {len(labs_data)} clean enterprise UNL labs with true EVE-NG startup-configs...")
...
for lab in labs_data:
    ...
    card_b64 = build_clean_enterprise_card(lab_title, lab_desc, table, tasks, cli_sol, verif, card_id="1", left=20, top=20, width=540)

    # Build Preloaded Configs XML for EVE-NG under <objects><configs> with base64 encoding
    configs_xml = []
    nodes_xml = []
    for idx, n in enumerate(nodes):
        ...
        if nname in cli_sol:
            cfg_text = "\n".join(cli_sol[nname])
        elif ntype == "vpcs" and "Endpoints (VPCS)" in cli_sol:
            cfg_text = "\n".join([c for c in cli_sol["Endpoints (VPCS)"] if nname in c])

        cfg_b64 = base64.b64encode(cfg_text.encode('utf-8')).decode('utf-8')
        configs_xml.append(f'''        <config id="{nid}">{cfg_b64}</config>''')

        # config="1" activates preloaded startup config in EVE-NG
        if ntype == "huaweiar1k":
            icon = "Switch L32.png" if n.get("role") == "switch" else "Router.png"
            nodes_xml.append(f'''      <node id="{nid}" name="{nname}" type="qemu" template="huaweiar1k" image="huaweiar1k-5.170" console="telnet" cpu="2" cpulimit="0" ram="4096" ethernet="6" uuid="{nguid}" qemu_options="-machine type=pc,accel=kvm -vga std -usbdevice tablet -boot order=cd -cpu host" qemu_version="2.12.0" qemu_arch="x86_64" qemu_nic="virtio-net-pci" delay="0" icon="{icon}" config="1" left="{nx}" top="{ny}">''')
        elif ntype == "huaweine40":
            nodes_xml.append(f'''      <node id="{nid}" name="{nname}" type="qemu" template="huaweine40" image="huaweine40-ne40" console="telnet" cpu="2" cpulimit="1" ram="2048" ethernet="12" uuid="{nguid}" qemu_options="-cpu host -machine type=pc-1.0,accel=kvm -serial mon:stdio -nographic -nodefconfig -nodefaults -rtc base=utc" qemu_arch="x86_64" icon="Router.png" config="1" left="{nx}" top="{ny}">''')
        elif ntype == "vpcs":
            nodes_xml.append(f'''      <node id="{nid}" name="{nname}" type="vpcs" template="vpcs" ethernet="1" console="" delay="0" icon="Desktop.png" config="1" left="{nx}" top="{ny}">''')
...
{full_nets_str}
  ...
  <objects>
    <textobjects>
      <textobject id="1" name="Lab_Card" type="text">
        <data>{card_b64}</data>
      </textobject>
    </textobjects>
    <configs>
...
print("All 32 clean enterprise Huawei UNL labs successfully updated with native EVE-NG startup configs!")
