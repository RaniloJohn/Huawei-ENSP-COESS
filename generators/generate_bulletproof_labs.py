# Bulletproof High-Contrast Div-Grid Layout Generator for Huawei 32-Lab Suite
# 1. Replaces HTML <table> with CSS Grid <div> to bypass EVE-NG table CSS white-override
# 2. Ensures all Device Names, Ports, IPs, and Links are 100% razor sharp and visible
# 3. Sets card width to 560px to prevent text clipping
# 4. Positions devices at x=640-960 with clear layout
...
def build_bulletproof_card(title, desc, table, tasks, cli_solution, verif_steps, card_id="1", left=20, top=20, width=540):
    # Div-Grid Table Rows (Immune to EVE-NG CSS table styling)
    grid_rows = ""
    for r in table:
...
# Build the 32 labs with optimal device coordinates
labs_master = []
...
# Generate remaining labs 6 to 32 with clear layout
additional_titles = [
...
print(f"Generating {len(labs_master)} bulletproof UNL labs...")

for lab in labs_master:
    lab_id = lab["id"]
    lab_title = lab["title"]
    lab_desc = lab["desc"]
    ...
    card_b64 = build_bulletproof_card(lab_title, lab_desc, table, tasks, cli_sol, verif, card_id="1", left=20, top=20, width=540)
...
print("All 32 bulletproof UNL labs successfully generated!")
