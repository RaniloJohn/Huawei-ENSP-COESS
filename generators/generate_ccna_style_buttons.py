# CCNA-Style Tabbed Buttons Generator for EVE-NG Huawei Labs
# Generates compact button tabs: [Description] [Tasks/Questions] [CLI Solution] [Verification]
# Starts closed by default so canvas is 100% clean, and opens upon click.
...
def build_ccna_style_buttons(title, desc, table, tasks, cli_solution, verif_steps, card_id="1", left=30, top=30):
    # Table rows
    table_rows = ""
    for r in table:
        table_rows += f"""<tr>
    ...
    for dev, cmds in cli_solution.items():
        ...
        cli_html += f"""
        <div style="margin-top:6px;background:#030712;border:1px solid #1e293b;border-radius:4px;padding:6px 8px;">
          <div style="color:#38bdf8;font-weight:bold;font-size:11px;border-bottom:1px solid #1e293b;padding-bottom:2px;margin-bottom:4px;">💻 {dev} Commands:</div>
    ...
    html = f"""<div id="customText{card_id}" class="customShape customText context-menu jtk-draggable" data-path="{card_id}" style="display:inline;position:absolute;left:{left}px;top:{top}px;z-index:1001;width:500px;">
  <div style="font-family:Segoe UI,Arial,sans-serif;font-size:11.5px;">
    
    <!-- Title Bar -->
    <div style="background:#0284c7;color:#ffffff;font-size:12px;font-weight:bold;padding:5px 10px;border-radius:5px 5px 0 0;letter-spacing:0.5px;display:flex;align-items:center;justify-content:space-between;">
      <span>📖 {title.replace('-', ' ').upper()}</span>
      <span style="font-size:9.5px;background:#0369a1;padding:1px 6px;border-radius:3px;">Huawei CCNA</span>
    </div>

    <div style="background:#090d16;border:1px solid #0284c7;border-top:none;border-radius:0 0 6px 6px;padding:8px;box-shadow:0 8px 24px rgba(0,0,0,0.6);">
      
      <!-- BUTTON 1: DESCRIPTION & ADDRESSING TABLE -->
      <details style="margin-bottom:5px;background:#1e293b;border:1px solid #334155;border-radius:4px;">
        <summary style="cursor:pointer;background:#0f172a;color:#38bdf8;font-weight:bold;font-size:11px;padding:5px 8px;border-radius:3px;user-select:none;">
          📘 [ Description &amp; IP Table ]
        </summary>
        <div style="padding:8px;background:#0b1120;">
          <div style="color:#cbd5e1;font-size:11px;margin-bottom:6px;line-height:1.35;">
          ...
          <table style="width:100%;border-collapse:collapse;font-size:10.5px;background:#030712;">
            <thead>
              <tr style="background:#1e293b;color:#ffffff;">
                ...
                <th style="padding:3px 6px;text-align:left;border:1px solid #374151;">IP Address</th>
      ...
      <details style="margin-bottom:5px;background:#1e293b;border:1px solid #334155;border-radius:4px;">
        <summary style="cursor:pointer;background:#0f172a;color:#facc15;font-weight:bold;font-size:11px;padding:5px 8px;border-radius:3px;user-select:none;">
          🎯 [ Tasks &amp; Questions ]
        </summary>
        <div style="padding:8px;background:#0b1120;">
          <ol style="padding-left:18px;margin:0;font-size:11px;">
            {tasks_html}
          </ol>
        </div>
      </details>

      <!-- BUTTON 3: STEP-BY-STEP CLI SOLUTION -->
      <details style="margin-bottom:5px;background:#1e293b;border:1px solid #334155;border-radius:4px;">
        <summary style="cursor:pointer;background:#0f172a;color:#4ade80;font-weight:bold;font-size:11px;padding:5px 8px;border-radius:3px;user-select:none;">
      ...
      <details style="background:#1e293b;border:1px solid #334155;border-radius:4px;">
        <summary style="cursor:pointer;background:#0f172a;color:#a7f3d0;font-weight:bold;font-size:11px;padding:5px 8px;border-radius:3px;user-select:none;">
          ✅ [ Verification &amp; Testing Guide ]
        </summary>
        <div style="padding:8px;background:#064e3b;color:#ecfdf5;font-size:10.5px;line-height:1.4;">
          {verif_html}
        </div>
      </details>
...
# Build the 32 labs with button-style tabs
labs_dataset = []

# LAB 1: DHCP & Easy-IP NAT
labs_dataset.append({
    "id": 1,
...
# Generate Labs 6 through 32 with button-style tabs
additional_titles = [
...
print(f"Generating {len(labs_dataset)} CCNA-style tabbed UNL labs...")

for lab in labs_dataset:
    lab_id = lab["id"]
    lab_title = lab["title"]
    lab_desc = lab["desc"]
    ...
    card_b64 = build_ccna_style_buttons(lab_title, lab_desc, table, tasks, cli_sol, verif, card_id="1", left=30, top=30)
...
{full_nets_str}
  ...
  <objects>
    <textobjects>
      <textobject id="1" name="CCNA_Style_Tabs" type="text">
        <data>{card_b64}</data>
      </textobject>
    </textobjects>
  </objects>
...
print("All 32 CCNA-style tabbed Huawei UNL labs successfully generated!")
